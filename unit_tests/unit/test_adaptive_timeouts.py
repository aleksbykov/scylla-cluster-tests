...
from sdcm.sct_config import SCTConfiguration, AdaptiveTimeoutOperationMultipliers
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(Operations.REMOVE_NODE, {"removenode": 3}, 3, id="removenode"),
        pytest.param(Operations.NEW_NODE, {"new_node": 2}, 2, id="new_node"),
        pytest.param(Operations.NEW_NODE, {"decommission": 4}, 1, id="missing-new_node"),
        pytest.param(Operations.SOFT_TIMEOUT, {"decommission": 4}, 1, id="unmapped-operation"),
        pytest.param(Operations.REMOVE_NODE, None, 1, id="no-config"),
    ],
)
def test_get_operation_timeout_factor(operation, config_value, expected):
    params = SCTConfiguration()
    params["adaptive_timeout_operation_multipliers"] = config_value
...
def test_decommission_multiplier_scales_hard_timeout_for_tablets(
    ...
    fake_node.parent_cluster.params["adaptive_timeout_operation_multipliers"] = {"decommission": 2}
    with adaptive_timeout(
        operation=Operations.DECOMMISSION, node=fake_node, stats_storage=adaptive_timeout_store
    ) as scaled_timeout:
...
def test_operation_multiplier_get_multiplier_returns_1_for_missing_key():
    model = AdaptiveTimeoutOperationMultipliers(decommission=4)
    assert model.get_multiplier("decommission") == 4
    assert model.get_multiplier("removenode") == 1
    assert model.get_multiplier("new_node") == 1
    assert model.get_multiplier("nonexistent") == 1
...
def test_operation_multipliers_env_var_loaded_by_sct_config(monkeypatch, env_value, expected_field, expected_val):
    ...
    multipliers = config.get("adaptive_timeout_operation_multipliers")
...
def test_operation_multipliers_env_var_null_preserves_default(monkeypatch):
    ...
    monkeypatch.delenv("SCT_ADAPTIVE_TIMEOUT_OPERATION_MULTIPLIERS", raising=False)
    config = SCTConfiguration()
...
def test_operation_multipliers_config_assignment_coerces(input_val, expected_key, expected_val):
    ...
    config = SCTConfiguration()
    config["adaptive_timeout_operation_multipliers"] = input_val
    multipliers = config.get("adaptive_timeout_operation_multipliers")
    assert isinstance(multipliers, AdaptiveTimeoutOperationMultipliers)
    assert getattr(multipliers, expected_key) == expected_val
