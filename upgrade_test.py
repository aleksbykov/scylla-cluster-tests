...
class UpgradeTest(FillDatabaseData, loader_utils.LoaderUtilsMixin):
    ...
    def test_generic_cluster_upgrade(self):
        ...
        self.run_raft_topology_upgrade_procedure()
        InfoEvent(message='Validate limited voters feature enabled').publish()
        self.validate_limited_voters_feature_enabled()
    ...
    def validate_limited_voters_feature_enabled(self):
        ...
        if all([not enabled for enabled in feature_state_per_node]):
            InfoEvent(message="Limited voters feature  is not supprted or enabled on all nodes").publish()
            return
        elif all(feature_state_per_node):
            InfoEvent(message="Step5.2 - Limited voters feature enabled on all nodes").publish()
            self.log.debug("Validate that number of voters less or equal number of nodes")
            number_of_nodes = len(self.db_cluster.nodes)
            num_of_non_voters = len(self.db_cluster.nodes[0].raft.get_group0_non_voters())
            if number_of_nodes <= 5 and num_of_non_voters > 0:
                InfoEvent(message='Number of voters are less than expected',
                          severity=Severity.ERROR).publish()
            elif number_of_nodes > 5 and num_of_non_voters == 0:
                InfoEvent(message='Number of voters higher than expected than expected',
                          severity=Severity.ERROR).publish()
            else:
                InfoEvent(message="Feature limited voters is enabled correctly")
            return
        else:
            InfoEvent(message="Limited voters feature not enabled on all nodes").publish()
