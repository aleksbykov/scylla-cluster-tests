    ...
    pipeline {
        ...
        environment {
         ...
         parameters {
            ...
            string(defaultValue: "gemini_test.GeminiTest.test_random_load",
                   description: '',
                   name: 'test_name')
            string(defaultValue: "test-cases/gemini/gemini_repo.yaml",
                   description: '',
                   name: 'test_config')
            ...
            string(defaultValue: '', description: '', name: 'oracle_scylla_version')
            string(defaultValue: '', description: '', name: 'oracle_ami_id')
            string(defaultValue: "1", description: '', name: 'gemini_seed')
                        ...
                        script {
                            wrap([$class: 'BuildUser']) {
                                dir('scylla-cluster-tests') {
                                    ...
                                    if [[ ! -z "${params.oracle_ami_id}" ]] ; then
                                        export SCT_AMI_ID_DB_ORACLE="${params.oracle_ami_id}"
                                    elif [[ ! -z "${params.oracle_scylla_version}" ]] ; then
                                        export SCT_GEMINI_VERSION="${params.oracle_scylla_version}"
                                    else
                                        echo "need to choose one of SCT_GEMINI_VERSION | SCT_AMI_ID_DB_ORACLE"
                                        exit 1
                                    fi
