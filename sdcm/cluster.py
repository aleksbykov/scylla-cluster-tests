...
class BaseNode(AutoSshContainerMixin):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def get_nodes_status(self) -> dict[BaseNode, dict]:
        nodes_status = {}
        try:
            if self.parent_cluster.params.get("use_zero_nodes") and SkipPerIssues("scylladb/scylladb#20909", self.parent_cluster.params):
...
class BaseScyllaCluster:  # pylint: disable=too-many-public-methods, too-many-instance-attributes, too-many-statements
    ...
    def get_nodes_status(self, verification_node: BaseNode | None = None) -> dict[str, dict[str, Any]]:
        """Get nodes status using nodetool or system table"""
        if self.params.get("use_zero_nodes") and SkipPerIssues("scylladb/scylladb#20909", self.params):
            return self.get_cluster_status(verification_node)
        return self.get_nodetool_status(verification_node)
