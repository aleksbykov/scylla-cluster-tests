...
class SCTConfiguration(BaseModel):
    ...
    #: First Scylla release whose Docker entrypoint accepts the `--dc`/`--rack` arguments.
    DOCKER_RACK_ARG_MIN_VERSION: ClassVar[str] = "2026.1.0-dev"
    ...
    def _validate_docker_simulated_racks(self) -> None:
        ...
        try:
            ...
            return
        if image_version < self.DOCKER_RACK_ARG_MIN_VERSION:
            raise ValueError(
                ...
                f"{self.DOCKER_RACK_ARG_MIN_VERSION}. Use a 2026.1+ image or set simulated_racks: 1."
            )
