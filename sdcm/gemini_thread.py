...
class GeminiToolEventsPublisher(FileFollowerThread):
    severity_mapping = {
        "INFO": "NORMAL",
        "DEBUG": "NORMAL",
        "WARN": "WARNING",
    ...
    def __init__(self, node, gemini_log_filename):
        super(GeminiToolEventsPublisher, self).__init__()
        self.gemini_log_filename = gemini_log_filename
        self.node = str(node)
        self.gemini_events = [GeminiToolLogEvent(type='geminievent', regex=r'{"L":"(?P<type>[A-Z]+?)".+"M":"(?P<message>[\w\s.,]+?)"}', severity=Severity.CRITICAL)]
...
class GeminiStressThread(object):
    ...
    def _run_gemini(self, node, loader_idx):
        ...
        GeminiToolEvent(type='start', cmd=gemini_cmd)
        try:
            with GeminiToolEventsPublisher(node=node, gemini_log_filename=log_file_name):
        ...
        GeminiToolEvent(type='finish', cmd=gemini_cmd,
