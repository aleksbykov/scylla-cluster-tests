...
class AbortExceptionHandler(ExceptionHandler):
    """Exception handler that always returns STOP strategy for testing purposes."""

    def handle_exception_during_collecting(
        self, collector: DiagnosticCollector, exception: Exception
    ) -> ExceptionStrategy:
        LOGGER.error("AbortExceptionHandler: Exception occurred in collector '%s': %s", collector.name, exception)
        return ExceptionStrategy.STOP

    def handle_exception_during_storing(
        self, collector: DiagnosticCollector, exception: Exception
    ) -> ExceptionStrategy:
        LOGGER.error(
            "AbortExceptionHandler: Exception occurred while storing diagnostics for collector '%s': %s",
            collector.name,
            exception,
        )
        return ExceptionStrategy.STOP
...
class RaiseExceptionOnCollectDiagnosticCollector(DiagnosticCollector):
    """A diagnostic collector that raises an exception during collect() for testing purposes."""

    def collect(self) -> Any:
        raise RuntimeError("Intentional exception during collect()")
